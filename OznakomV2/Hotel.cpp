...
int Hotel::AddInfo(std::vector<Hotel> &data)
{
	...
	while (true) {
		...
		else std::cout << "Incorrect value, try again:";
	}
	switch (variant)
	{
...
void Hotel::UpdateDataInFile(std::vector<Hotel> &data)
{
	...
	for (int i{}; i < data.size(); i++)
	{
		...
		fout << data[i].GetId() << '\t' << data[i].corpus << '\t' << data[i].room << '\t' << favors[0] << '\t' << favors[1] << '\t' << favors[2] << '\t' << favors[3] << '\t' << favors[4] << '\t' << favors[5] << '\t' << data[i].GetCost() << '\t' << data[i].GetFullName() << '\t' << data[i].GetPhoneNumber() << '\t' << data[i].GetEMail() << '\t' << data[i].date.GetFullDate() << '\t' << data[i].date.getDay() << '\t' << data[i].date.getMonth() << '\t' << data[i].date.getYear() << '\t' << data[i].GetFullCost() << '\n';
